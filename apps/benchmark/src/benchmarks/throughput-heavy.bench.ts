...
export const throughputHeavyBenchmark: BenchmarkScenario = {
  name: "throughput-heavy",
  description:
    "Throughput test with heavy workflows (10+ steps) - measures CPU-bound capacity",

  async run(config: BenchmarkConfig): Promise<BenchmarkResult> {
    const runtime = config.runtime === "both" ? "bun" : config.runtime;
    const runner = new BenchmarkRunner(config, runtime);

    return await runner.run("throughput-heavy", async (engine, metrics) => {
      ...
      const maxInFlight = config.concurrency ?? 500; // Lower for CPU-bound
      const reportIntervalMs = 5000; // Report every 5 seconds
      const stepCount = 10; // Heavy workflows with many steps
      ...
      console.log("\n🔥 CPU-Bound Throughput Test (HEAVY WORKFLOWS)");
      console.log("─".repeat(80));
      console.log("Phase 1: Active Load Generation (CPU-intensive processing)");
      ...
      const fireWorkflows = async () => {
        ...
        const firstInstance = await workflow.create({
          params: {
            stepCount, // Many steps for CPU-bound test
            delayMs: 0,
          },
        });
        ...
        while (Date.now() < creationPhaseEnd) {
          ...
          if (tracker && now - lastReportTime >= reportIntervalMs) {
            ...
            const stepThroughput = completionRate * stepCount; // Steps/second
            ...
            console.log(
              `[${elapsed.toFixed(1)}s] ` +
                ...
                `Completion: ${completionRate.toFixed(0)} wf/s (${stepThroughput.toFixed(0)} steps/s)`
            );
      ...
      const stepThroughput = overallThroughput * stepCount; // Steps/second
      ...
      console.log(
        `  Processing Rate:     ${overallThroughput.toFixed(2)} wf/s (${stepThroughput.toFixed(0)} steps/s sustained)`
      );
