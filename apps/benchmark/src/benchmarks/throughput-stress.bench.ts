...
export const throughputStressBenchmark: BenchmarkScenario = {
  name: "throughput-stress",
  description:
    "Maximum sustained throughput test - fire workflows continuously and measure workflows/sec",

  async run(config: BenchmarkConfig): Promise<BenchmarkResult> {
    const runtime = config.runtime === "both" ? "bun" : config.runtime;
    const runner = new BenchmarkRunner(config, runtime);

    return await runner.run("throughput-stress", async (engine, metrics) => {
      ...
      const maxInFlight = config.concurrency ?? 1000; // Max concurrent workflows
      ...
      console.log("\n🔥 Throughput Stress Test (THROTTLED)");
      console.log("─".repeat(80));
      console.log("Phase 1: Active Load Generation (throttled creation rate)");
      ...
      const fireWorkflows = async () => {
        ...
        while (Date.now() < creationPhaseEnd) {
          ...
          if (tracker && now - lastReportTime >= reportIntervalMs) {
            ...
            currentInFlight = status.inProgress; // Update currentInFlight
